...
#define MAX_STATES 100
#define MAX_ALPHABET_SIZE 26 // Assuming lowercase English letters 'a' to 'z'
...
int main() {
    ...
    // --- NEW Example DFA Input (will be minimized) ---
    ...
    original_dfa.num_states = 6;
    original_dfa.initial_state = 0;

    original_dfa.num_alphabet = 2;
    original_dfa.alphabet[0] = 'a';
    original_dfa.alphabet[1] = 'b';

    original_dfa.is_final[0] = false;
    original_dfa.is_final[1] = false;
    original_dfa.is_final[2] = true;
    original_dfa.is_final[3] = true;
    original_dfa.is_final[4] = false;
    original_dfa.is_final[5] = false;

    // Transitions:
    int a_idx = get_alphabet_index(&original_dfa, 'a');
    int b_idx = get_alphabet_index(&original_dfa, 'b');

    original_dfa.transitions[0][a_idx] = 1;
    original_dfa.transitions[0][b_idx] = 2;

    original_dfa.transitions[1][a_idx] = 4;
    original_dfa.transitions[1][b_idx] = 3;

    original_dfa.transitions[2][a_idx] = 5;
    original_dfa.transitions[2][b_idx] = 4;

    original_dfa.transitions[3][a_idx] = 5;
    original_dfa.transitions[3][b_idx] = 4;

    original_dfa.transitions[4][a_idx] = 5;
    original_dfa.transitions[4][b_idx] = 4;

    original_dfa.transitions[5][a_idx] = 5;
    original_dfa.transitions[5][b_idx] = 5;

    print_dfa(&original_dfa, "Original DFA");
